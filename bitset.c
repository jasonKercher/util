...
Bitset_Uint bitset_get_uint(Bitset_Max bs, unsigned idx, unsigned width)
{
	return bs & bit_mask(idx, width) >> (idx);
}

/* NOTE: not compatible with BIG endian machine */
Bitset_Int bitset_get_int(Bitset_Max bs, unsigned idx, unsigned width)
{
	...
	if (bit(width - 1) & raw) {
		...
		Bitset_Max negative_mask = bit_mask(width - 1, MAX_BITS - (width - 1));
		raw |= negative_mask;
	}
