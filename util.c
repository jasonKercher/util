...
void*
heap_resize(void* data, long size) {
...
///
inline uint16_t
bytes_to_u16(uint8_t b0, uint8_t b1) {
	uint16_t val = b1;
	val <<= 8;
	return val | b0;
}
...
int
div_round_up(int dividend, int divisor) {
	return dividend / divisor + dividend % divisor;
}

int
str2ulongbase(unsigned long* ret, const char* s, int base) {
	char* endptr = NULL;
	errno        = 0;
...
int
str2ulong(unsigned long* ret, const char* s) {
...
int
str2longbase(long* ret, const char* s, int base) {
	char* endptr = NULL;
	errno        = 0;
...
int
str2long(long* ret, const char* s) {
	if (s[0] == '0' && s[1] == 'x' && isdigit(s[2])) {
...
int
str2double(double* ret, const char* s) {
	char* endptr = NULL;
	errno        = 0;
	*ret         = strtod(s, &endptr);
...
int
charncount(const char* s, char c, unsigned n) {
	int      count = 0;
	unsigned i     = 0;
	for (; s[i] != '\0' && i < n; ++i)
		if (s[i] == c)
...
int
strhaschar(const char* s, char c) {
	int i = 0;
	for (; s[i] != '\0'; ++i)
		if (s[i] == c)
			return true;
 ...
 */
void
getnoext(char* dest, const char* filename) {
	char* lastdot = NULL;
	char* lastsep = NULL;
